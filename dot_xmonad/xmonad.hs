...
myKeys c =
  ...
  [ ("M-S-s", addName "Select window and focus" $ (selectWindow emConfig{bgCol=green, borderCol=green} >>= (`whenJust` windows . W.focusWindow)))
  , ("M-S-;", addName "Select window and kill" $ (selectWindow emConfig{bgCol=red, borderCol=red} >>= (`whenJust` killWindow)))
  , ("M-S-j", addName "Screenshot focused window" $ spawn "maim -i (xdotool getactivewindow) | xclip -selection clipboard -t image/png")
