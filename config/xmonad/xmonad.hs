...
import XMonad.Actions.EasyMotion(selectWindow, EasyMotionConfig(..), textSize, ChordKeys ( AnyKeys) )
...
myFont      = "xft:Space Mono Nerd Font:size=15:bold:antialias=true"
...
emConfig = def
             { txtCol=base3
	     , cancelKey=xK_q
	     , emFont="xft:Overpass Nerd Font:size=35"
	     , overlayF= textSize
...
myKeys c =
  ...
  [ ("M-S-C-q r", addName "Recompile XMonad"               $ spawn "xmonad --recompile")
  , ("M-S-C-q t", addName "Restart XMonad"                 $ spawn "xmonad --restart")
  , ("M-S-C-q w", addName "Recompile and Restart XMonad"   $ sequence_ [spawn ("xmonad --recompile"), spawn ("xmonad --restart")])
  , ("M-S-C-q ;", addName "Quit XMonad"                    $ io exitSuccess)
  , ("M-<Space>",     addName "Run prompt"                     $ spawn myLauncher)
  ...
  [ ("M-S-C-z", addName "Switch to workspace 1"    $ (windows $ W.greedyView $ myWorkspaces !! 0))
  , ("M-S-C-x", addName "Switch to workspace 2"    $ (windows $ W.greedyView $ myWorkspaces !! 1))
  , ("M-S-C-c", addName "Switch to workspace 3"    $ (windows $ W.greedyView $ myWorkspaces !! 2))
  , ("M-S-C-d", addName "Switch to workspace 4"    $ (windows $ W.greedyView $ myWorkspaces !! 3))
  , ("M-S-C-=", addName "Switch next workspace"    $ (nextWS))
  , ("M-S-C--", addName "Switch previous workspace"    $ (prevWS))
  ]

  ^++^ subKeys "Send window to workspace"
  [ ("M-S-C-k", addName "Send to workspace 1"    $ (windows $ W.shift $ myWorkspaces !! 0))
  , ("M-S-C-h", addName "Send to workspace 2"    $ (windows $ W.shift $ myWorkspaces !! 1))
  , ("M-S-C-,", addName "Send to workspace 3"    $ (windows $ W.shift $ myWorkspaces !! 2))
  , ("M-S-C-.", addName "Send to workspace 4"    $ (windows $ W.shift $ myWorkspaces !! 3))]
  ...
  [ ("M-S-C-s", addName "Select window and focus" $ (selectWindow emConfig{bgCol=green, borderCol=green} >>= (`whenJust` windows . W.focusWindow)))
  , ("M-S-C-;", addName "Select window and kill" $ (selectWindow emConfig{bgCol=red, borderCol=red} >>= (`whenJust` killWindow)))
  , ("M-S-C-j", addName "Screenshot focused window" $ spawn "maim -i (xdotool getactivewindow) | xclip -selection clipboard -t image/png")
  , ("M-S-C-n", addName "Launch Warpd" $ spawn "warpd --hint")
  , ("M-S-C-p", addName "Promote focused window to master"  $ promote)]
...
-}
  ...
  [ ("M-S-C-<Return>", addName "Launch terminal"   $ spawn (myTerminal))
  , ("M-S-C-b", addName "Launch web browser"       $ spawn (myBrowser))
  , ("M-S-C-<Space>", addName "Launch Launcher"    $ spawn (myLauncher))
  , ("M-S-C-e", addName "Launch emacs"             $ spawn ("emacsclient -nc -a 'emacs'")) ]

  -- Switch layouts
  ^++^ subKeys "Switch layouts"
  [ ("M-S-C-l", addName "Switch to next layout"   $ sendMessage NextLayout)
  , ("M-S-C-f", addName "Toggle noborders/full" $ sendMessage (MT.Toggle NBFULL) >> sendMessage ToggleStruts)]
...
-}
  ...
  ^++^ subKeys "Scratchpads"
  [ ("M-S-C-o", addName "Toggle scratchpad terminal"   $ namedScratchpadAction myScratchPads "terminal")]
...
myStartupHook = do 
    ...
    spawnOnce "warpd &"
    spawnOnce "udiskie &"
    spawnOnce "/home/hawkbane/.local/share/gem/ruby/3.0.0/bin/fusuma -d &"
